...
interface AutocompleteInputAPIProps {
  dataType: 'tickers' | 'sectors' | 'topics' | 'keywords';
...
const AutocompleteInputAPI: React.FC<AutocompleteInputAPIProps> = ({
  dataType,
  selectedItems,
  onAdd,
  onRemove,
  ...
  const [suggestions, setSuggestions] = useState<MarketDataItem[]>([]);
  const [popularSuggestions, setPopularSuggestions] = useState<MarketDataItem[]>([]);
  ...
  const loadPopularSuggestions = async () => {
    try {
      ...
      switch (dataType) {
        case 'tickers':
          data = await marketDataService.searchTickers('', 8);
          break;
        case 'sectors':
          data = await marketDataService.getSectors();
          break;
        case 'topics':
          data = await marketDataService.getTopics();
          break;
        case 'keywords':
          data = await marketDataService.getKeywords();
          break;
      }
      setPopularSuggestions(data.slice(0, 8));
    } catch (error) {
  ...
  const searchSuggestions = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        ...
        switch (dataType) {
          case 'tickers':
            data = await marketDataService.searchTickers(query, maxItems);
            break;
          case 'sectors':
            data = await marketDataService.getSectors(query);
            break;
          case 'topics':
            data = await marketDataService.getTopics(query);
            break;
          case 'keywords':
            data = await marketDataService.getKeywords(query);
            break;
        }

        // Filter out already selected items
        const filtered = data.filter(item => !selectedItems.includes(item.value));
    ...
    [dataType, maxItems, selectedItems]
  );
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Popular Suggestions */}
      {!showSuggestions && selectedItems.length === 0 && popularSuggestions.length > 0 && (
        <div className="mt-4">
...
export default AutocompleteInputAPI;